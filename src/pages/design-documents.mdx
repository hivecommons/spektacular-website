...
description: "What a design document is, when to reach for one instead of putting the detail in a spec, and how Spektacular resolves, reads and references them."
---
...
<Hero
  ...
  sub="The worked design a feature is built to, kept where your team already keeps it, and bound to the spec that needs it."
/>
...
<Section heading="What a design document is">
  ...
  flow, a data format, or a worked example of one. It is the detail a spec needs
  to be built correctly but cannot carry without becoming unreadable.

  Spektacular owns the **reference**, not the document. It resolves a declared
  ...
  stands. A design Spektacular authors with you is different: it carries the
  same lifecycle record every spec and plan carries, so you can see where it
  stands, which spec's conversation produced it, and which specs depend on it.

  That is why design documents are declared rather than filed. You point
  Spektacular at wherever your designs already live, give each place a name, and
...
<Section heading="When to use one instead of a spec section" surface>
  ...
  - **Would it make the spec unreadable inline?** If a one-line steer captures
    it, that steer belongs in the spec and there is no design document to write.
  ...
  - A preference the planner may adapt is **technical direction**, and stays in
    the spec as a one-line steer.
  - A worked design that would swamp the spec is a **design document**, and the
    spec points at it.
...
<Section heading="Working a design out with Spektacular">
  ...
  settled in conversation but never written down, Spektacular can help you work
  ...
  file behind. A design can be authored before any spec exists; the reference
...
<Section heading="What an authored design records" surface>
  ...
  A design Spektacular authors carries the same frontmatter block every spec
  and plan carries:
  ...
  - **Document status**: where it stands, one of `draft`, `final`,
    `superseded` or `archived`. The same four values every other Spektacular
    document uses.
  - **Originating spec**: the spec whose conversation produced it, when there
    was one. A design authored outside a spec simply does not carry this.
  - **Referencing specs**: every spec that references it, kept in step
  ...
  difference between the two kinds, and it is also how Spektacular tells them
...
<Section heading="How they relate to specs and plans">
  ...
  **A spec records references.** Each one names both the source it belongs to
  and the document within that source, so it can always be resolved back to a
  real place. References live in the spec's own record rather than its prose,
  which is what makes them validated when written and durable afterwards: they
  survive the spec being rewritten.
  ...
  **Once the work ships, the design is a historical record.** Nothing requires a
  ...
  A design document holds no back-links, so any number of specs may reference
  the same design and it outlives the feature that introduced it. Here is how a
  spec carries its references:
...
<Section heading="Working with designs from the command line">
  ...
  <Fragment slot="sub">
    ...
    directly, as they already do for specs, plans and changelog records.
  ...
  Write one that Spektacular authored with you, stamping its lifecycle record.
  Name the spec the conversation belonged to, and set a status, if either
  applies:
  ...
  `design author` for one Spektacular wrote with you. Re-running `design author`
  on an existing authored design updates it in place, keeping its original
  capture date and the specs already referencing it. Running `design write` over
  ...
  A design that a spec still references is not removed. The refusal names every
  spec that references it and gives you the step to clear each one, and neither
  the document nor any spec is changed by the attempt, so a spec can never be
  ...
  spec carries and deliberately leaves the document where it is. `design delete`
  ...
  Record a reference on a spec, or remove one. A reference naming a source the
  ...
  Ask what a spec references, and whether each reference resolves:
...
<Section heading="When a reference cannot be found" surface>
  ...
  succeeds, so a single call shows every reference a spec carries at once, with
  ...
  and it reads the same way. It names the resolved location, every spec that
  ...
  A design the project already had, rather than one Spektacular authored, carries
  no record of referencing specs, so there is nothing to find and it is removed
  like any other document.

  Recording or removing a reference is a change to two documents, the spec and
  ...
  exactly as it was, so a spec and a design are never left contradicting each
  other. In the rare case that restoring the spec also fails, that is reported
...
<Section heading="Why it works this way">
  ...
  **The spec stays readable because the design is referenced, not absorbed.**
  The complaint that motivated design documents was specs that became unusable
  once a worked API shape was pasted into them. A reference keeps the spec at
  ...
  **Storage is declared, not imposed.** Teams already keep designs somewhere: a
  ...
  move into a Spektacular-owned directory would be the one thing guaranteed to
